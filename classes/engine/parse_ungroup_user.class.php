...
class parse_ungroup_user extends tokenizer {

    public static $samples;

    public function __construct($remainder, &$parser) {
        parent::__construct($remainder, $parser);
        self::$samples = "UNGROUP USER username:<username> FROM idnumber:<groupidnum>\n\n";
        self::$samples = "UNGROUP USER username:<username> FROM name:\"<groupname>\" IN COURSE shortname:<courseshortname>\n";
    }
    ...
    public function parse() {
        ...
        $this->trace('...Start parse ungroup user '.$this->remainder);
        ...
        if (preg_match($pattern, $this->remainder, $matches)) {
            ...
            if ($target == 'current') {
                $context->groupuserid = $target;
            } else {
                $context->groupuserid = $identifier->parse($target);
            }
