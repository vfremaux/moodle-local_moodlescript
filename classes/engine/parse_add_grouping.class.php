...
class parse_add_grouping extends tokenizer {

    public static $samples;

    public function __construct($remainder, &$parser) {
        parent::__construct($remainder, $parser);
        self::$samples = "ADD GROUPING <groupingname> IF NOT EXISTS TO idnumber:<courseidnum> IDENTIFIED BY <groupingidnumber>\n\n";
        self::$samples = "ADD GROUPING <groupingname> TO idnumber:<courseidnum> IDENTIFIED BY <groupingidnumber>\n";
    }
    ...
    public function parse() {
        $this->trace('...Start parse add grouping '.$this->remainder);
        ...
        if (preg_match($pattern, $this->remainder, $matches)) {
            ...
            $handler = new \local_moodlescript\engine\handle_add_grouping();

            $context = new StdClass;
            $context->groupingname = $matches[1];
            ...
            if (!empty($matches[4])) {
                $context->groupingidnumber = $matches[3];
            }
