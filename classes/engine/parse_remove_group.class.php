...
class parse_remove_group extends tokenizer {

    public static $samples;

    public function __construct($remainder, &$parser) {
        parent::__construct($remainder, $parser);
        self::$samples = "REMOVE GROUP name:<groupname> FROM idnumber:<courseidnum>\n\n";
        self::$samples = "REMOVE GROUP idnumber:<groupidnumber> FROM idnumber:<courseidnum>\n\n";
        self::$samples = "REMOVE GROUP id:<groupid> FROM idnumber:<courseidnum>\n\n";
        self::$samples = "REMOVE GROUP name:<groupname> FROM shortname:<courseshortname>\n\n";
        self::$samples = "REMOVE GROUP name:<groupname> FROM id:<courseid>\n\n";
    }
    ...
    public function parse() {
        $this->trace('...Start parse remove group '.$this->remainder);
        ...
        if (preg_match($pattern, $this->remainder, $matches)) {
            ...
            $handler = new \local_moodlescript\engine\handle_remove_group();
            ...
            if ($target == 'current') {
                ...
                $context->groupid = $identifier->parse($target);
            }
