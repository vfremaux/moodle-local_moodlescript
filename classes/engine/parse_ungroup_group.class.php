...
class parse_ungroup_group extends tokenizer {

    public static $samples;

    public function __construct($remainder, &$parser) {
        parent::__construct($remainder, $parser);
        self::$samples = "UNGROUP GROUP idnumber:<groupidnumber> FROM idnumber:<groupingidnum>\n\n";
        self::$samples = "UNGROUP GROUP name:\"<groupname>\" FROM name:\"<groupname>\" IN COURSE shortname:<courseshortname>\n\n";
    }
    ...
    public function parse() {
        $this->trace('...Start parse ungroup group '.$this->remainder);
        ...
        if (preg_match($pattern, $this->remainder, $matches)) {
            ...
            $handler = new \local_moodlescript\engine\handle_ungroup_group();
            ...
            if (!empty($matches[3])) {
                ...
                $context->groupcourseid = $cidentifier->parse($ctarget);
                $allgood = true;
            }

            $user = $matches[1];
            $identifier = new \local_moodlescript\engine\parse_identifier('group', $this->logger);
            if ($target == 'current') {
            ...
            $target = $matches[2];
            $identifier = new \local_moodlescript\engine\parse_identifier('grouping', $this->logger);
            if ($target == 'current') {
